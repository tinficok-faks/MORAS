class Parser:
	def __init__(self):
		...
		self._labels = []  # lista ua pracenje definiranih labels-a

	def _is_label_defined(self, label):
	...
	def _parseLine(self, line, n):
		...
		if l[0] == "push":
			if not l[2].isdigit():
				self._flag = False
				Parser._error("Parser", n, "Invalid argument for push, non-negative integer")
				return ""
			else:
				if len(l) == 3:
			...
			if not l[2].isdigit():
				self._flag = False
				Parser._error("Parser", n, "Invalid argument for pop, non-negative integer")
				return ""
			else:
				if len(l) == 3:
					return "//" + " ".join(l) + "\n" + self._pop(l[1], l[2], n)
			...
			if l[0] == "label" and len(l) == 2:
				self._labels.append(l[1])  # doda label na definirane labels
				return "//" + " ".join(l) + "\n" + self._label(l[1], n)
			elif l[0] == "goto" and len(l) == 2:
				if not self._is_label_defined(l[1]):
					self._flag = False
